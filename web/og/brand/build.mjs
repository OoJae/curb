...
import { COLORS as C, MARK, MARK_SMALL, markPaths, dots, sector, fmt } from './geometry.mjs';
...
// ── Geometry for the /brand page's live specimens (inline SVG: rotate the arc, close the C) ──────────────────────
{
  const h = (MARK.stroke * MR) / 2;
  const cx = MS / 2;
  const small = markPaths(8, 8, gR, MARK_SMALL);
  const ts =
    `// Generated by web/og/brand/build.mjs from geometry.mjs — do not edit by hand.\n` +
    `// The mark in a ${MS}×${MS} box centred on (${cx}, ${cx}), R = ${MR}; the 16 px glyph variant in a 16×16 box.\n` +
    `export const MARK_BOX = ${MS};\n` +
    `export const MARK_CENTRE = ${cx};\n` +
    `/** Degrees from due east to each end of the C (the gap is twice this). */\n` +
    `export const C_HALF_GAP = ${MARK.cHalfGap};\n` +
    `export const MARK_C = '${m.c}';\n` +
    `export const MARK_ARC = '${m.arc}';\n` +
    `/** The C split at due west: upper half and lower half (for the "confirmed" motion, each rotates to close). */\n` +
    `export const MARK_C_UPPER = '${sector(cx, cx, MR - h, MR + h, MARK.cHalfGap, 180)}';\n` +
    `export const MARK_C_LOWER = '${sector(cx, cx, MR - h, MR + h, 180, 360 - MARK.cHalfGap)}';\n` +
    `export const MARK_RING = '${m.ring}';\n` +
    `export const GLYPH_C = '${small.c}';\n` +
    `export const GLYPH_ARC = '${small.arc}';\n` +
    `export const GLYPH_RING = '${small.ring}';\n` +
    `export const GLYPH_DOTS = '${dots(8, 8, gR, 8, gStroke / 2)}';\n`;
  await writeFile(join(WEB, 'src', 'pages', 'brand', 'mark-data.ts'), ts);
  written.push(['src/pages/brand/mark-data.ts', ts.length]);
}
