/**
 * COPIED verbatim from services/asp/src/reopen.ts, itself a copy of services/keeper/src/reopen.ts. Each
 * service is built standalone from its own directory, so shared pure code is copied rather than imported
 * across packages; keep the copies in step. This is the rule curb_next_reopen answers with, so its answer
 * is the instant the keeper commits a Scorecard row's settleAfter to.
 *
