/**
 * COPIED verbatim from services/asp/src/closureCalendar.ts (with its test, closureCalendar.test.ts). Each
 * service is built standalone from its own directory, so shared pure code is copied rather than imported
 * across packages; keep the copies in step. curb_next_reopen answers from `buildCalendar`, so the free
 * reopen time and the paid calendar at api.curb.markets cannot disagree for the same issuer bytes.
 *
