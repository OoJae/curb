/**
 * COPIED verbatim from services/asp/src/regime.ts, itself a copy of services/keeper/src/regime.ts. Each
 * service is built standalone from its own directory, so shared pure code is copied rather than imported
 * across packages; keep the copies in step. curb-mcp uses only `sessionAt` and the types (through
 * closureCalendar.ts); the rest is kept so that, below this header, it matches the keeper's line for line.
 *
