/**
 * COPIED from services/keeper/src/sources/chain.ts. Each service is built standalone from its own directory,
 * so shared pure code is copied rather than imported across packages; keep the copies in step.
 *
 * One deliberate difference: error strings name an endpoint by endpointLabel (its position and origin),
 * never by its full URL. Here those strings reach the public /healthz, and a keyed provider (drpc,
 * alchemy, infura) carries its API key in the URL's path or query.
 *
...
/**
 * How an RPC endpoint is named anywhere a person or a public page can read it: "rpc[1] https://host". The
 * origin keeps what an operator needs to tell endpoints apart; the path, query and userinfo, where keyed
 * providers put the key, never appear. A string that is not a URL is named by position alone.
 */
...
export async function attestedRounds(rpcs: string[], clock: string, fromBlock: number, toBlock: number): Promise<AttestedRound[]> {
  ...
  for (let b = fromBlock; b <= toBlock; b += LOG_RANGE) {
    ...
    for (const [i, url] of rpcs.entries()) {
      try {
        const head = heads[i];
        if (head < end) { reasons.push(`${endpointLabel(url, i)} head ${head} < ${end}`); continue; }
