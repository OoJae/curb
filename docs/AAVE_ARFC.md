...
5. On the day of posting, re-read every live figure and update or re-date it:
   ...
   - CurbCredit `reserve()`, `ltvFor(wTCENTx)`, DepthCert `nextId()`, and both allowlists. "Today, two disclosed
     team wallets" and "curb-desk is also an allowlisted borrower" must still be true; `docs/DEPLOYMENTS.md` says
     curb-desk leaves the borrower registry after the 25 Sept demo. "The first borrow was refused" stays true.
